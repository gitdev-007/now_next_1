$frontendPath = "C:\Users\Dev Tinker\Desktop\now_next\frontend"
...
foreach ($file in $files) {
    ...
    if (Test-Path $filePath) {
        try {
            ...
            # Simple approach: match from footer start to footer end
            # The old footer starts with "  <!-- FOOTER -->" and ends with "</footer>"
            # followed by newlines and "  <!-- Modals Section -->"
            $pattern = '(  <!-- FOOTER -->\s*<footer class="bg-gray-900.*?</footer>)(\r?\n)(\r?\n)(\r?\n)  <!-- Modals Section -->'

            if ($content -match $pattern) {
                $replacement = "$newFooter$($Matches[2])$($Matches[3])  <!-- Modals Section -->"
